...
const tableName = 'kurejai'; // Lentelės pavadinimas
...
async function selectId(id, cb) {
    ...
    const result = await pool.query(`SELECT * FROM ${tableName} WHERE id_kurejai = $1`, [ id ]);
...
async function deleteId(id, cb) {
    ...
    const result = await pool.query(`DELETE FROM ${tableName} WHERE id_kurejai = $1`, [ id ]);
...
async function update(values, cb) {
    ...
    const result = await pool.query(`UPDATE ${tableName} SET pavadinimas = $1, logotipas = $2, hipersaitas = $3 WHERE id_kurejai = $4`, 
