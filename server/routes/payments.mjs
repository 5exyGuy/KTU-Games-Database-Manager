...
async function selectAll(data, cb) {
    const result = await pool.query(`SELECT mokejimai.id_mokejimai, v.slapyvardis as moketojas, u.id_uzsakymai as uzsakymas, mokejimai.data, mokejimai.kaina FROM mokejimai
...
async function insert(values, cb) {
    ...
    values.data = new Date(values.data).getTime();
...
async function update(values, cb) {
    ...
    const result = await pool.query(`UPDATE mokejimai SET tipas = $1, kaina = $2, data = $3, fk_uzsakymaiid_uzsakymai = $4, fk_vartotojaiid_vartotojai = $5 WHERE id_mokejimai = $6`, 
    [ 
        values.tipas, values.kaina, values.data,
        values.fk_uzsakymaiid_uzsakymai, values.fk_vartotojaiid_vartotojai,
        values.id_mokejimai
